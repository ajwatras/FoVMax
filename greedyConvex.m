function [camera_pose,covered_poly,camera_views] = greedyConvex(FOV_rads,plane_of_stitching, coverage_reg, covered_poly,T_bounds,R_bounds,scene_depth,thresh,camera_pose)
%   GREEDYCONVEX calculates the calculates the optimal camera pose for a camera to be added to a cross shaped camera array.
%   where optimal is determined by the total area of the largest convex
%   region visible to the camera array.
...
if (length(covered_poly) > 1)
    [current_area,covered_poly_con] = peelPotato(covered_poly');
else
    current_area = 0;
end
                ...
                if (length(covered_poly) > 1)
                    ...
                    if (max(overlap_area) - thresh > 0)
                        ...
                        if ~isempty(temp_clip_x)
                            ...
                            [covered_area,con_corners] = peelPotato(new_covered');
                        else
                            new_covered = 0;
                            covered_area = 0;
                            %combined_area = 0;
...
% [con_area,hull_shape] = peelPotato(covered_poly');
% 
% if length(hull_shape) > 1
%     figure(4)
%     hold on
%     fill(covered_poly(:,1),covered_poly(:,2),'b')
%     fill(hull_shape(1,:),hull_shape(2,:),'r')
% end
% sorted_out(end,:)
% hull_shape
